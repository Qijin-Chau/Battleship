...
function randomSearch ()
    ...
    while done == false
        [x, y] = selectNewRandomXYLocation();
        QueryOpponentBoard(x, y);
        %displayBoard(GAME.shipList)
        done = allShipsAreSunk();
    end
end

function neighborhoodSearch ()
%   neighborhoodSearch shoots randomly until it scores a hit, then searches
%   around that point until a ship is sunk.
%   This function first uses a new function, randomSearch2, that searches
%   randomly and terminates once it scores a hit. After that, the sinkShip
%   function will sink whatever ship was hit, then the function checks to
%   see if all ships are sunk. Once all ships are sunk, the function ends. 
    global GAME;
    done = false; 
    while done == false
        [x, y] = randomSearch2();
...
function [x, y] = randomSearch2 ()
%   randomSearch2 searches randomly like randomSearch, only this function
%   terminates once it scores a hit and not once the board is complete. 
%   The function selects random (x, y) locations and queries them, until
%   the queried position scores a hit.
    global GAME;
    done = false;
    while done == false
        [x, y] = selectNewRandomXYLocation();
        [isHit, ~, ~] = QueryOpponentBoard(x, y);
        done = isHit;
    end
end

function sinkShip(x, y)
%   sinkShip sinks a ship on whatever point the function lands upon. 
%   This functions utilizes four other functions that check in four
%   different coordinate directions relative to the point where the initial hit is
%   scored, and the function terminates once a ship is sunk. It does this
%   by checking if the ship is sunk after every direction is checked.
    global GAME;
    isShipSunk = false;
        if isShipSunk == false
            [isShipSunk] = checkRight(x, y);
        end
        if isShipSunk == false
            [isShipSunk] = checkLeft(x, y);
        end 
        if isShipSunk == false
            [isShipSunk] = checkUp(x, y);
        end
        if isShipSunk == false
            [isShipSunk] = checkDown(x, y);
        end
end

function [isShipSunk] = checkRight(x, y)
%   checkRight shoots to the right of the initial hit until it is no longer
%   able to shoot or it is illogical to shoot.
%   This function takes the (x, y) position of the hit as input, and
%   returns whether or not the ship is sunk as output. First, it sets the x
%   position as a so that the original x value of the hit isn't changed. It
%   checks to see if the point to the right of the hit is valid or queried,
%   and if it is valid and not queried, it takes a shot. If either of these
%   conditions are not met, the function terminates and returns an output
%   of isShipSunk = false. If they are met, the function will keep
%   querrying positions to the right until either it doesn't score a hit,
%   the position is invalid, the position has already been queried, or the
%   ship is sunk. Once it terminates, it returns an output stating whether
%   or not the ship is sunk.
    global GAME;
    a = x;
    isHit = true;
    isQueried = false;
    isShipSunk = false;
    [isValid] = isPositionValid(a+1, y);
    if isValid
        [isQueried, ~] = isPositionQueried(a+1, y);
    end
    if isValid == true && isQueried == false
        while isHit == true && isValid == true && isQueried == false && isShipSunk == false
            [isHit, isShipSunk, ~] = QueryOpponentBoard(a+1, y);
            a = a+1;
            [isValid] = isPositionValid(a+1, y);
            if isValid
                [isQueried, ~] = isPositionQueried(a+1, y);
    ...
    elseif isValid == false || isQueried == true
        isShipSunk = false;
    end
end

function [isShipSunk] = checkLeft(x, y)
%   checkLeft shoots to the left of the initial hit until it is no longer
%   able to shoot or it is illogical to shoot.
%   This function operates in the same exact way as checkRight, except it
%   goes to the left of the initial hit. 
    global GAME;
    a = x;
    isHit = true;
    isQueried = false;
...
function [isShipSunk] = checkUp(x, y)
%   checkUp shoots above the initial hit until it is no longer
%   able to shoot or it is illogical to shoot.
%   This function operates in the same exact way as checkRight, except it
%   goes above the initial hit. 
    global GAME;
    a = y;
    isHit = true;
    isQueried = false;
    isShipSunk = false;
    [isValid] = isPositionValid(x, a+1);
    if isValid
        [isQueried, ~] = isPositionQueried(x, a+1);
    end
    if isValid == true && isQueried == false
        while isHit == true && isValid == true && isQueried == false && isShipSunk == false
            [isHit, isShipSunk, ~] = QueryOpponentBoard(x, a+1);
            a = a+1;
            [isValid] = isPositionValid(x, a+1);
            if isValid
                [isQueried, ~] = isPositionQueried(x, a+1);
...
function [isShipSunk] = checkDown(x, y)
%   checkDown shoots below the initial hit until it is no longer
%   able to shoot or it is illogical to shoot.
%   This function operates in the same exact way as checkRight, except it
%   goes below the initial hit. 
    global GAME;
    a = y;
    isHit = true;
    isQueried = false;
